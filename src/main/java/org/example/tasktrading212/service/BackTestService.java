...
public class BackTestService {
    ...
    public BacktestResult runBacktest(Long userId, BigDecimal initialBalance, StrategyType strategyType, BacktestPeriod period) {
        ...
        List<PricePoint> prices = historyClient.getHistoricalPrices(SYMBOL, config.interval(), config.limit());
            ...
            switch (signal) {
                case BUY -> {
                    if (usdtBalance.compareTo(BigDecimal.ZERO) > 0) {
                        BigDecimal quantity = usdtBalance.divide(pricePoint.price(), 2, RoundingMode.DOWN);
