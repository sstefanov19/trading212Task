...
class TradingBotTest {
    ...
    @Test
    void start_shouldNotRunWhenFactoryFails() {
        when(strategyFactory.createStrategy(STRATEGY_TYPE)).thenThrow(new RuntimeException("bad config"));
    ...
    @Test
    void pause_shouldSkipEvaluation() {
        when(strategyFactory.createStrategy(STRATEGY_TYPE)).thenReturn(strategy);
        ...
        assertTrue(tradingBot.isPaused());
        assertTrue(tradingBot.isRunning());
    ...
    @Test
    void resume_shouldContinueEvaluation() {
        when(strategyFactory.createStrategy(STRATEGY_TYPE)).thenReturn(strategy);
        when(tradingService.getUsdtBalance(USER_ID)).thenReturn(USDT_BALANCE);
        when(tradingService.getBtcHoldings(USER_ID)).thenReturn(BTC_HOLDINGS);
        when(strategy.evaluate(PRICE, USDT_BALANCE, BTC_HOLDINGS)).thenReturn(TradeSignal.HOLD);
        ...
        verify(strategy).evaluate(PRICE, USDT_BALANCE, BTC_HOLDINGS);
    }

    @Test
    void stop_shouldResetPausedState() {
        when(strategyFactory.createStrategy(STRATEGY_TYPE)).thenReturn(strategy);
    ...
    @Test
    void isPaused_shouldReturnFalseInitially() {
        assertFalse(tradingBot.isPaused());
    }
}